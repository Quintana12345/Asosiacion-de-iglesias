...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Verificar si ya existe un sector con el mismo nombre en la misma zona
        $stmt = $db->prepare("SELECT id FROM sectores WHERE nombre = ? AND zona_id = ?");
        $stmt->execute([$nombre, $zona_id]);
        if ($stmt->fetch()) {
            throw new Exception('Ya existe un sector con ese nombre en esta zona');
        }

        // Insertar nuevo sector
        $stmt = $db->prepare("INSERT INTO sectores (nombre, descripcion, zona_id, jefe_sector_id, activo, fecha_creacion) VALUES (?, ?, ?, ?, ?, NOW())");
        $stmt->execute([$nombre, $descripcion, $zona_id, $jefe_sector_id, $activo]);
...
<head>
    ...
    <title>Agregar Sector - <?php echo SITE_NAME; ?></title>
        ...
        <div class="content-container">
            <div class="form-container">
                <div class="form-header">
                    <h2><i class="fas fa-plus-circle"></i> Registrar Nuevo Sector</h2>
                </div>
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group full-width">
                                    ...
                                    <textarea id="descripcion" name="descripcion" rows="3" 
                                              class="form-textarea"
                                              placeholder="Descripción breve del sector, ubicación, características..."><?php echo htmlspecialchars($_POST['descripcion'] ?? ''); ?></textarea>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <select id="jefe_sector_id" name="jefe_sector_id" class="form-select">
                                        ...
                                        <?php foreach ($jefesSector as $jefe): ?>
                                            <option value="<?php echo $jefe['id']; ?>" 
                                                <?php echo ($_POST['jefe_sector_id'] ?? '') == $jefe['id'] ? 'selected' : ''; ?>>
