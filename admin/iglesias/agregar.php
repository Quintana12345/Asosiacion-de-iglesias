...
// Obtener zona_id del POST si existe (para mantener la selección después del submit)
$zona_id_selected = $_POST['zona_id'] ?? '';
...
try {
    ...
    // Obtener sectores si hay una zona seleccionada
    if (!empty($zona_id_selected)) {
        $stmt = $db->prepare("SELECT id, nombre FROM sectores WHERE zona_id = ? AND activo = 1 ORDER BY nombre");
        $stmt->execute([$zona_id_selected]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Verificar si ya existe una iglesia con el mismo nombre en el mismo sector
        $stmt = $db->prepare("SELECT id FROM iglesias WHERE nombre = ? AND sector_id = ?");
        $stmt->execute([$nombre, $sector_id]);
        if ($stmt->fetch()) {
            throw new Exception('Ya existe una iglesia con ese nombre en este sector');
        }

        // Insertar nueva iglesia (SOLO campos que existen en tu tabla)
        $stmt = $db->prepare("INSERT INTO iglesias (nombre, direccion, telefono, sector_id, pastor_id, activo, fecha_creacion) VALUES (?, ?, ?, ?, ?, ?, NOW())");
        $stmt->execute([$nombre, $direccion, $telefono, $sector_id, $pastor_id, $activo]);

        $success = '✅ Iglesia creada exitosamente';
...
<head>
    ...
    <title>Agregar Iglesia - <?php echo SITE_NAME; ?></title>
...
<body class="dashboard-body">
        ...
        <div class="content-container">
            <div class="form-container">
                <div class="form-header">
                    <h2><i class="fas fa-plus-circle"></i> Registrar Nueva Iglesia</h2>
                </div>
                                    ...
                                    <select id="sector_id" name="sector_id" class="form-select" required>
                                        ...
                                        <?php foreach ($sectores as $sector): ?>
                                            <option value="<?php echo $sector['id']; ?>" 
                                                <?php echo ($_POST['sector_id'] ?? '') == $sector['id'] ? 'selected' : ''; ?>>
                            ...
                            <div class="form-grid">
                                <div class="form-group full-width">
                                    ...
                                    <textarea id="direccion" name="direccion" rows="3" 
                                              class="form-textarea"
                                              placeholder="Dirección completa de la iglesia..."><?php echo htmlspecialchars($_POST['direccion'] ?? ''); ?></textarea>
                                </div>
                                    ...
                                    <select id="pastor_id" name="pastor_id" class="form-select">
                                        ...
                                        <?php foreach ($pastores as $pastor): ?>
                                            <option value="<?php echo $pastor['id']; ?>" 
                                                <?php echo ($_POST['pastor_id'] ?? '') == $pastor['id'] ? 'selected' : ''; ?>>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Cargar sectores si hay una zona seleccionada
