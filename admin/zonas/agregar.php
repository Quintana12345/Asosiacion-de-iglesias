...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $db->prepare("SELECT id FROM zonas WHERE nombre = ?");
        $stmt->execute([$nombre]);
        if ($stmt->fetch()) {
        ...
        // Insertar nueva zona
        $stmt = $db->prepare("INSERT INTO zonas (nombre, descripcion, jefe_zona_id, activo, fecha_creacion) VALUES (?, ?, ?, ?, NOW())");
        $stmt->execute([$nombre, $descripcion, $jefe_zona_id, $activo]);

        $success = '✅ Zona creada exitosamente';
...
<head>
    ...
    <title>Agregar Zona - <?php echo SITE_NAME; ?></title>
        ...
        <div class="content-container">
            <div class="form-container">
                <div class="form-header">
                    <h2><i class="fas fa-plus-circle"></i> Registrar Nueva Zona</h2>
                </div>
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group full-width">
                                    ...
                                    <textarea id="descripcion" name="descripcion" rows="3" 
                                              class="form-textarea"
                                              placeholder="Descripción breve de la zona, ubicación, características..."><?php echo htmlspecialchars($_POST['descripcion'] ?? ''); ?></textarea>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <select id="jefe_zona_id" name="jefe_zona_id" class="form-select">
                                        ...
                                        <?php foreach ($jefesZona as $jefe): ?>
                                            <option value="<?php echo $jefe['id']; ?>" 
                                                <?php echo ($_POST['jefe_zona_id'] ?? '') == $jefe['id'] ? 'selected' : ''; ?>>
